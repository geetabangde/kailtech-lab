...
export default function RolesDatatable() {
  ...
  const fetchRoles = async () => {
    try {
      ...
      const response = await axios.get("rolemanagment/get-roles");
    
    // console.log("API response:", response.data); // debug

    if (response.data.status && Array.isArray(response.data.data)) {
      setRoles(response.data.data); // ✅ correct assignment
    } else {
  ...
  deleteRows: (rows) => {
    ...
    setRoles((old) => old.filter((row) => !rowIds.includes(row.id)));
  },
  setTableSettings
},
    filterFns: {
        ...
        >
          ...
          >
            ...
            >
              ...
              <SelectedRowsActions table={table} title="Role List" showDelete={false} />
