...
import { mockNcRecords } from "./data";

// ----------------------------------------------------------------------

export default function NonConfirmingRecordDashboard() {
  const navigate = useNavigate();
  const permissions = getStoredPermissions();

  // Route security gate: check permission ID 451 or standard bypass
  const hasAccess =
    permissions.includes(451) ||
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-profile-nc-v1",
    {},
  );
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-profile-nc-v1",
    {},
  );
  ...
  const fetchRecords = useCallback(async () => {
    const endpoints = [
      "profile/myNclistData.php",
      "profile/get-nc-list",
      "profile/non-confirming-records",
      "profile/nc-list",
      "profile/myNclistData",
    ];
  ...
  }, [hasAccess, fetchRecords]);

  // Alertify prompt launcher
  const handleOpenPrompt = (record) => {
    setPromptRecord(record);
    setPromptValue(record.remark || "");
  ...
  const handleRemarkSubmit = async (e) => {
    ...
    const payload = {
      taskid: promptRecord.ref_no || promptRecord.id,
      remark: promptValue,
      status: "processing",
    };
    ...
    if (success) {
      ...
      setNcData((prev) =>
        prev.map((c) =>
          c.ref_no === promptRecord.ref_no ? { ...c, remark: promptValue } : c
        )
      );
    } else {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            onClick={() => handleOpenPrompt(row.original)}
          >
            Remark
          </Button>
        </div>
  ...
  if (!hasAccess) {
    return (
      <Page title="Non Confirming List">
            ...
            <div className="grow overflow-auto p-0 scrollbar-thin">
              ...
              >
                <THead className="bg-gray-55 dark:bg-dark-800 border-b border-gray-200 dark:border-dark-600">
                  ...
                  {/* Header Row 2: DataTable Column-Specific Filtering Inputs */}
                  <Tr className="bg-gray-100/70 dark:bg-dark-800/80 border-b border-gray-250 dark:border-dark-600">
                    {table.getFlatHeaders().map((header) => {
                      const isFilterable = header.id !== "s_no" && header.id !== "view_action" && header.id !== "completion_percentage" && header.id !== "status";
                ...
                <TBody>
                  {loading ? (
                    ...
                    <Tr>
                      ...
                      >
                        No non-confirming records found matching criteria
                      </Td>
                    </Tr>
                  )}
                </TBody>
          ...
          <div className="h-full w-full max-w-xl bg-white p-6 shadow-xl dark:bg-dark-800 flex flex-col justify-between overflow-y-auto border-l border-gray-200 dark:border-dark-600 animate-slide-in">
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-5 text-sm">
                <div>
                  <p className="text-xs text-gray-400 font-bold dark:text-dark-300">RECORD DATE</p>
                  <p className="font-semibold text-gray-800 dark:text-dark-100 mt-1">{selectedRecord.date}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400 font-bold dark:text-dark-300">SOURCE</p>
                  <p className="font-semibold text-gray-800 dark:text-dark-100 mt-1">{selectedRecord.source}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400 font-bold dark:text-dark-300">CONCERN PERSON</p>
                  <p className="font-semibold text-gray-800 dark:text-dark-100 mt-1">{selectedRecord.concern_person}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400 font-bold dark:text-dark-300">TYPE</p>
                  <p className="font-semibold text-gray-800 dark:text-dark-100 mt-1">{selectedRecord.type}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400 font-bold dark:text-dark-300">COMPLETION STATUS</p>
                  <p className="font-semibold text-emerald-600 dark:text-emerald-450 mt-1">{selectedRecord.completion_percentage}%</p>
                </div>
                <div className="sm:col-span-2 border-t border-gray-150 pt-3 dark:border-dark-500">
                  <p className="text-xs text-gray-400 font-bold dark:text-dark-300">NATURE & DETAILS OF NC</p>
                  <p className="text-gray-700 dark:text-dark-250 mt-1 leading-relaxed whitespace-pre-wrap">{selectedRecord.nature_details}</p>
                </div>
                <div className="sm:col-span-2 border-t border-gray-150 pt-3 dark:border-dark-500">
                  ...
                  <p className="text-emerald-700 bg-emerald-50/50 border border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-450 dark:border-emerald-900/30 p-3 rounded-lg mt-1 font-medium leading-relaxed whitespace-pre-wrap">
                    {selectedRecord.remark || "No task remarks yet."}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-gray-250 pt-4 dark:border-dark-500 flex justify-end gap-3">
              <Button
                ...
                onClick={() => {
                  ...
                  handleOpenPrompt(selectedRecord);
                }}
              >
                Update Remark
              </Button>
            ...
            <form onSubmit={handleRemarkSubmit} className="space-y-4">
              <div className="flex flex-col gap-2">
                ...
                <textarea
                  ...
                  placeholder="Enter record assessment remark..."
