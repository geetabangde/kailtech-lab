...
import { AddDispatchDetailModal } from "./AddDispatchDetailModal";
  ...
  if (column.id === 'status') {
    return (
      ...
      >
        ...
        <option value="Pending For Checklist">Pending For Checklist</option>
        <option value="Pending For Approval">Pending For Approval</option>
        <option value="Pending For Dispatch">Pending For Dispatch</option>
        <option value="Din Rejected">Din Rejected</option>
      </select>
    );
  }
...
export default function DinList() {
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-profile-din-list",
    {},
  );
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-profile-din-list",
    {},
  );
  ...
  const [selectedDinForDispatch, setSelectedDinForDispatch] = useState(null);
  ...
  const fetchDinList = async () => {
    try {
      setLoading(true);
      const response = await axios.get("profile/din-list");
      ...
      console.error("Error fetching profile din list data:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      setTableSettings,
      openDispatchModal: (din) => setSelectedDinForDispatch(din),
    },
    filterFns: {
      fuzzy: fuzzyFilter,
    },
      ...
      <Page title="Din List">
        <div className="flex h-60 items-center justify-center rounded-xl border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
          <p className="text-sm font-medium text-red-600 dark:text-red-400">
            Access Denied - Permission 301 required
          </p>
        </div>
      </Page>
    );
  ...
  return (
    <Page title="Din List">
            ...
            <div className="card-header flex flex-col items-start justify-between gap-4 border-b border-gray-200 p-4 dark:border-dark-500 sm:flex-row sm:items-center sm:p-5">
              ...
              <div className="flex w-full flex-wrap items-center justify-end gap-3 sm:w-auto">
                ...
                {permissions.includes(303) && (
                  <Button
                    component={Link}
                    to="/dashboards/profile/din-list/add-din"
                    color="primary"
                    variant="filled"
      ...
      <AddDispatchDetailModal 
        din={selectedDinForDispatch} 
        onClose={() => setSelectedDinForDispatch(null)} 
        onSuccess={() => {
          setSelectedDinForDispatch(null);
          fetchDinList();
        }}
      />
    </Page>
