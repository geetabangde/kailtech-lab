// ViewItemizedBill.jsx
// Route: /dashboards/accounts/testing-invoices/view-itemized/:id
// PHP port of: viewitemizedbill.php
//
// KEY differences from ViewDetailedInvoice:
//   1. Items come from `invoiceitemcalibraton` table (not `invoicerow`)
//   2. Item description = name(brand) + "CCL Updation-{brnno}"
//   3. Qty/Amount logic:
//        meter_option == 1 → qty = meter,  amount = rate × meter
//        else              → qty = 1,      amount = rate
//   4. BRN No row is commented out (not shown) in the bottom-left cell
//   5. API: GET /accounts/view-itemized-invoice/:id
//   6. Date field uses invoicedate (not approved_on)
//
// Shared logic with other views:
...
function printInvoice(templateProps, withLH, logoSrc, pageTitle) {
  const bodyHtml = renderToStaticMarkup(
    <ItemizedBillPrintTemplate {...templateProps} withLH={withLH} />,
  );
...
// ─── Inline style tokens (zero Tailwind / oklch — safe for html2canvas) ──────
const S = {
  wrap: {
...
// ─── PDF Print Template ───────────────────────────────────────────────────────
function ItemizedBillPrintTemplate({
  inv,
  addr,
  items,
  signUrl,
  ...
  return (
    <div style={{ ...S.wrap, paddingTop: !withLH ? 130 : 16 }}>
      ...
      {/* Items — from invoiceitemcalibraton table */}
      <table style={S.table}>
        <thead>
          <tr>
      ...
      {/* Bottom: Remark + company info (BRN No commented out) + summary */}
      <table style={S.table}>
        <tbody>
          <tr>
            <td
            ...
            >
              {/* PHP: BRN No is commented out for itemized view */}
              {inv.remark?.trim() && (
                <div>
...
// Main Component
// ─────────────────────────────────────────────────────────────────────────────
export default function ViewItemizedBill() {
  const { id } = useParams();
  ...
  //   items: [{ name, brand, brnno, meter_option, meter, rate, id }],
  //   signature_image, digital_signature } }
  const load = useCallback(async () => {
    if (!id) return;
    setLoading(true);
    try {
      const res = await axios.get(`/accounts/view-itemized-invoice/${id}`);
    ...
    <Page title="View Itemized Bill">
      <div className="transition-content px-[var(--margin-x)] pb-10">
        ...
        >
          ...
          {/* ── Items table — from invoiceitemcalibraton ── */}
          ...
          {/* ── Bottom table: Remark + company info + summary ── */}
          <table className="dark:border-dark-500 mt-2 w-full border-collapse border border-gray-400 text-xs">
            <tbody>
              <tr>
