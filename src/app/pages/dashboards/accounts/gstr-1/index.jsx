...
import { columns, getSupplierType, getPlaceOfSupply, getGstRate } from "./columns";
...
export default function GSTR1() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      // Filter Credit Notes client-side by status, customer, date range, and intra-state status
      const filteredCnData = rawCnData.filter((cn) => {
        ...
        // Intra-state filter (CGST+SGST): has CGST/SGST amount OR state code is "23" / missing
        const hasCgstSgst = Number(cn.cgstamount || 0) > 0 || Number(cn.sgstamount || 0) > 0;
        const isIntraState = String(cn.statecode || "").trim() === "23" || !cn.statecode;
        if (!hasCgstSgst && !isIntraState) return false;
  ...
  const exportToExcel = () => {
    ...
    if (currentType === "CREDIT_NOTE") {
      ...
      fileName = `GSTR1_CreditNotes_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      fileName = `GSTR1_Export_${new Date().toISOString().slice(0, 10)}.xlsx`;
      exportData = filteredData.map((row) => ({
      ...
      fileName = `GSTR1_${typeLabel}_SGST_${new Date().toISOString().slice(0, 10)}.xlsx`;
      exportData = filteredData.map((row) => ({
        "GSTIN/UIN of Recipient": row.gstno || "",
        "Receiver Name": row.custname || "",
        "Invoice number": row.invoiceno || "",
  ...
  const totals = filteredData.reduce(
    ...
    { finaltotal: 0, subtotal2: 0, cgstamount: 0, sgstamount: 0, cessamount: 0, roundoff: 0 },
  );
