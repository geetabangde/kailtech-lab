...
export default function AcceptSample() {
  ...
  useEffect(() => {
    const fetchDropdowns = async () => {
      ...
      try {
        // Using /master/list-lab as it's the standard reliable endpoint for labs across modules
            ...
            >
              <div className="table-wrapper min-w-full grow overflow-x-auto">
                <Table
                  ...
                  className="w-full table-auto text-left rtl:text-right text-[11px] [&_th]:whitespace-nowrap [&_td]:whitespace-normal [&_td]:break-words [&_th]:px-1 [&_td]:px-1"
                >
                  <THead>
                        ...
                        <Tr>
                          {headerGroup.headers.map((header) => (
                            <Th
                              key={header.id}
                              className={clsx(
                                "bg-gray-200 font-semibold uppercase text-xs text-gray-800 dark:bg-dark-800 dark:text-dark-100 first:ltr:rounded-tl-lg last:ltr:rounded-tr-lg first:rtl:rounded-tr-lg last:rtl:rounded-tl-lg",
