...
export function RowActions({ row, table }) {
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      toast.success("CMC Scope Sheet deleted successfully ", {
        duration: 1000,
      });
    } catch (error) {
      console.error("Delete failed:", error);
      setDeleteError(true);
      toast.error("Failed to delete CMC Scope Sheet ", {
        duration: 2000,
      });
    } finally {
  ...
  return (
    <>
      <div className="flex items-center justify-center gap-2">
        {/* Edit Button */}
        {permissions.includes(84) && (
          <Link
            to={`/dashboards/calibration-operations/cmc-scope-sheet/edit/${row.original.id}`}
