...
export default function ViewAllLeaves() {
  ...
  const fetchLeaves = useCallback(async (filters = {}) => {
    try {
      setLoading(true);
      const response = await axios.get("/hrm/view-all-leaves-list", {
        params: filters
      });
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-all-leaves",
    {},
  );
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-all-leaves",
    {},
  );
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      deleteRows: (rows) => {
        ...
        setLeavesData((old) => old.filter((row) => !rowIds.includes(row.id)));
      },
      setTableSettings,
    },
    filterFns: {
  ...
  useDidUpdate(() => table.resetRowSelection(), [leavesData]);
  useLockScrollbar(tableSettings.enableFullScreen);

  // ✅ Loading UI
  if (loading) {
    return (
      <Page title="View All Leaves">
