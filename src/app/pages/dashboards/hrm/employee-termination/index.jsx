...
export default function EmployeeTermination() {
  ...
  const fetchTerminations = useCallback(async () => {
    try {
      setLoading(true);
      const response = await axios.get("/hrm/employee-termination-list");
      ...
      console.error("Error fetching termination list:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-termination",
    {},
  );
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-termination",
    {},
  );
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      deleteRows: (rows) => {
        ...
        setTerminations((old) => old.filter((row) => !rowIds.includes(row.id)));
      },
      setTableSettings,
    },
    filterFns: {
  ...
  useDidUpdate(() => table.resetRowSelection(), [terminations]);
  useLockScrollbar(tableSettings.enableFullScreen);

  // ✅ Loading UI
  if (loading) {
    return (
      <Page title="Employee Termination">
