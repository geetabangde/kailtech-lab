...
export default function PoliciesDatatable() {
  ...
  const fetchPolicies = async () => {
    try {
      setLoading(true);
      const response = await axios.get("/hrm/policy-list");

      if ((response.data.status === "success" || response.data.status === true) && Array.isArray(response.data.data)) {
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-policies",
    {},
  );
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-policies",
    {},
  );
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      deleteRows: (rows) => {
        ...
        setPolicies((old) => old.filter((row) => !rowIds.includes(row.id)));
      },
      setTableSettings,
    },
    filterFns: {
  ...
  useDidUpdate(() => table.resetRowSelection(), [policies]);
  useLockScrollbar(tableSettings.enableFullScreen);

  // ✅ Loading UI
  if (loading) {
    return (
      <Page title="View Policies">
