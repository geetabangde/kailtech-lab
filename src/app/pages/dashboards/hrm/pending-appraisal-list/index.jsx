...
export default function PendingAppraisalList() {
  ...
  const fetchAppraisals = useCallback(async () => {
    try {
      setLoading(true);
      const response = await axios.get("/hrm/pending-appraisal-list");
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-pending-appraisal",
    {},
  );
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-pending-appraisal",
    {},
  );
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      deleteRows: (rows) => {
        ...
        setAppraisals((old) => old.filter((row) => !rowIds.includes(row.id)));
      },
      setTableSettings,
    },
    filterFns: {
  ...
  useDidUpdate(() => table.resetRowSelection(), [appraisals]);
  useLockScrollbar(tableSettings.enableFullScreen);

  // ✅ Loading UI
  if (loading) {
    return (
      <Page title="Pending Appraisal List">
