...
export default function OrdersDatatableV1() {
  ...
  useEffect(() => {
    ...
    const loc = (params.get("caliblocation") || "Lab").toLowerCase();
    const acc = (params.get("calibacc") || "Nabl").toLowerCase();

    // Permission Guard Logic
    if (loc === "lab" && acc === "nabl" && !permissions.includes(368)) {
      navigate("/dashboards");
      return;
    }
    if (loc === "lab" && acc === "non nabl" && !permissions.includes(97)) {
      navigate("/dashboards");
      return;
    }
    if (loc === "site" && acc === "nabl" && !permissions.includes(369)) {
      navigate("/dashboards");
      return;
    }
    if (loc === "site" && acc === "non nabl" && !permissions.includes(370)) {
      navigate("/dashboards");
      return;
    }

    fetchModes(params.get("caliblocation") || "", params.get("calibacc") || ""); // pass dynamic values
  }, [location.search, navigate, permissions]);
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-inward-entry-lab-index",
    {},
  );
